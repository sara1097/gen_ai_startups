...
from app.src.chat_schemas.response_schema import ChatRequest, ChatResponse
from app.src.engine.services.chat_service import ChatService
from app.src.engine.core.providers.providers_factory import ProviderFactory

# Configure basic logging (can be replaced with structlog for production)
logging.basicConfig(
    level=logging.INFO,
...
app = FastAPI(
    title="Startup AI Service",
    description="AI-Powered System for generating startup ideas in the MENA region.",
    version="1.0.0",
)
...
async def value_error_handler(request: Request, exc: ValueError):
    ...
    return JSONResponse(
        status_code=422,
        content={"error": "Data Validation Error", "details": str(exc)},
    )
...
async def runtime_error_handler(request: Request, exc: RuntimeError):
    ...
    return JSONResponse(
        status_code=503,
        content={"error": "Service Unavailable", "details": "Upstream AI service failed. Please try again later."},
    )
...
def get_chat_service() -> ChatService:
    ...
    Instantiates the factory and service only when needed.
    """
    factory = ProviderFactory()
    return ChatService(
        llm_provider=factory.llm,
        retriever=factory.retriever,
        intent_classifier=factory.intent_classifier
    )
...
async def chat_endpoint(
    ...
    """
    Main chat endpoint for processing user input and generating startup ideas.
    """
    logger.info(f"Received chat request for conversation: {request.conversationId}")
    
    # The service layer handles all business logic, LLM orchestration, and validation
    result = chat_service.process_message(request)
    
    logger.info(f"Successfully processed request for conversation: {request.conversationId}")
    return result
